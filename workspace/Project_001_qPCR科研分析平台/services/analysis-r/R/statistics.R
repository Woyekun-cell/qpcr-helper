recommend_statistics <- function(design, contrast_mode = "selected") {
  switch(
    ...
    one_way = if (identical(contrast_mode, "all_pairwise")) {
      list(
        primary = "welch_anova",
...
two_group_statistics <- function(data, design, calibrator_group, correction) {
  ...
  if (design == "paired_two_group") {
    ...
    test <- stats::t.test(treatment$deltaCt, control$deltaCt, paired = TRUE)
    method <- "Paired t-test"
  } else {
    test <- stats::t.test(treatment$deltaCt, control$deltaCt, var.equal = FALSE)
...
nonparametric_two_group_statistics <- function(data, design, calibrator_group, correction) {
  groups <- unique(as.character(data$groupId))
  if (length(groups) != 2) stop(sprintf("%s requires exactly two groups", design))
  treatment_group <- setdiff(groups, calibrator_group)
  control <- data[data$groupId == calibrator_group, , drop = FALSE]
...
games_howell_contrasts <- function(data) {
  ...
  rows <- lapply(pairs, function(pair) {
    ...
    critical <- stats::qtukey(0.95, nmeans = length(groups), df = degrees_freedom) / sqrt(2)
...
one_way_statistics <- function(data) {
  ...
  list(
    ...
    contrasts = games_howell_contrasts(data),
    diagnostics = list(
      analysis_unit = "biological replicate",
      scale = "delta Ct",
...
dunn_statistics <- function(data, calibrator_group, contrast_mode, correction) {
...
dunnett_statistics <- function(data, calibrator_group) {
  ...
  intervals <- stats::confint(fit)$confint
  ...
  contrasts <- data.frame(
    ...
    statistic = unname(fit_summary$test$tstat),
    degrees_freedom = unname(stats::df.residual(model)),
    p_value = unname(fit_summary$test$pvalues),
...
run_statistics <- function(
  ...
  if (design == "two_way") {
    return(factorial_statistics(data, correction))
  }
  if (design == "repeated_time") {
    return(repeated_statistics(data, correction))
  }
