...
import { runRPreview } from "@/lib/r-client";
...
export async function POST(request: Request) {
  ...
  if (!allowRequest(`figure:${ip}`, 120, 10 * 60 * 1000)) {
  ...
  try {
    ...
    const payload = previewRequestSchema.parse(JSON.parse(text));
    return NextResponse.json(await runRPreview(payload), {
      headers: { "Cache-Control": "no-store" }
    });
  } catch (error) {
    return NextResponse.json({
      error: "FIGURE_PREVIEW_FAILED",
      message: error instanceof Error ? error.message : "Figure preview failed"
    }, { status: 422 });
  }
}
