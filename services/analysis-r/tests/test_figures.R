...
time_samples <- expand.grid(
  subjectId = c("control-1", "control-2", "treated-1", "treated-2"),
  time = c(0, 6, 24),
  KEEP.OUT.ATTRS = FALSE,
  stringsAsFactors = FALSE
)
time_samples$sampleId <- paste(time_samples$subjectId, time_samples$time, sep = "-")
time_samples$biologicalReplicateId <- time_samples$subjectId
time_samples$groupId <- factor(ifelse(grepl("^control", time_samples$subjectId), "control", "treated"), levels = c("control", "treated"))
time_samples$targetGene <- "GENE1"
time_samples$foldChange <- 2^(ifelse(time_samples$groupId == "control", time_samples$time / 24 * 0.2, time_samples$time / 24 * 1.8))
time_plot <- build_expression_plot(time_samples, plot_type = "time")
time_geoms <- vapply(time_plot$layers, function(layer) class(layer$geom)[1], character(1))
if (!all(c("GeomLine", "GeomPoint", "GeomErrorbar") %in% time_geoms)) stop("Time plots must include subject lines, group points and error bars")
time_build <- tryCatch(ggplot2::ggplot_build(time_plot), error = function(error) stop(sprintf("Time plot build failed: %s", conditionMessage(error))))
time_point_fills <- unique(unlist(lapply(time_build$data[vapply(time_plot$layers, function(layer) class(layer$geom)[1], character(1)) == "GeomPoint"], function(layer) layer$fill)))
if (!all(c("#4F6B45", "#D98268") %in% time_point_fills)) stop("Time plot points must use stable group fill colors")
...
multi_gene_bar_build <- ggplot2::ggplot_build(multi_gene_bar)
multi_gene_bar_scale <- multi_gene_bar_build$plot$scales$get_scales("fill")
multi_gene_bar_colors <- multi_gene_bar_scale$palette(2)
if (!identical(unname(toupper(multi_gene_bar_colors)), c("#264653", "#E76F51"))) {
  stop("Multi-gene bar colors must assign one stable color to each group")
}
if (!identical(multi_gene_bar_scale$get_limits(), c("control", "treated"))) {
  stop("Multi-gene bar fill scale must be keyed by group, not gene-group series")
}
...
expected_group_order <- c("control", "treated")
if (!identical(left_to_right_limits, expected_group_order)) {
  stop("Multi-gene colors must follow the stable group order across facets")
}
